module.exports = async function moveZettel(params) {
    const PERMANENT_FOLDER = "04 - Permanent/";
    ...
    if (activeFileFolder === PERMANENT_FOLDER) {
        new Notice("File is already in permanent folder");
        return;
    }
    ...
    await params.app.vault.rename(
        activeFile,
        `${PERMANENT_FOLDER}${activeFileName}.${activeFileExt}`
    );
}
