...
<%*
// Extract the start of the year based on the file title
...
tR += '>[!picture]- Pictures\n';
tR += Array(days_in_year).fill(null).map((x, i) => 
  `>![[${moment(start_of_year).add(i, 'days').format('YYYY-MM-DD[]#YYYY-MM-DD')}]]`).join("\n") + '\n';
%>
...
AND date >= date(<% moment(tp.file.title, "YYYY").startOf("year").format("YYYY-MM-DD") %>)
AND date <= date(<% moment(tp.file.title, "YYYY").endOf("year").format("YYYY-MM-DD") %>)
...
AND date <= date(<% moment(tp.file.title, 'YYYY').startOf('year').add(1,'months').add(-1, 'days').format("YYYY-MM-DD") %>)
